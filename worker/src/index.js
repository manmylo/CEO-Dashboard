...
async function askClaude(question, history, dashboardData, apiKey) {
  const messages = [
  ...
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 1024,
      system: `You are a business analyst assistant for Gearevo, a Malaysian knife/gear retailer selling through Shopify, Shopee, and TikTok Shop. You're answering questions from Gearevo's own staff about their business data, shown inside their internal dashboard.

${BUSINESS_CONTEXT}

You are given a snapshot of the dashboard's current data as JSON. Answer the user's question using ONLY that data — do not invent figures, trends, or product names that aren't present. If the data doesn't contain what's needed to answer, say so plainly rather than guessing. Keep answers concise and business-focused, in plain English. Use RM for currency figures, formatted to 2 decimal places.
...
export default {
  async fetch(request, env) {
    ...
    try {
      const reply = await askClaude(question, body.history, firestoreResult.data, env.ANTHROPIC_API_KEY);
